...
/// The external-storage metrics a lang SDK reports on a workflow activation completion must
/// reach core's workflow-task duration log. This exercises the full extraction/threading path
/// (completion proto -> `TaskStorageMetrics` -> `mark_wft_complete`), which the direct-call unit
/// tests of `log_workflow_task_duration` deliberately bypass.
#[tokio::test]
async fn tmprl1104_completion_storage_metrics_reach_log() {
    use std::sync::Mutex;
    use temporalio_common::{
        protos::coresdk::common::ExternalStorageMetrics,
        telemetry::{
            CoreLog, CoreLogConsumer, Logger, TelemetryOptions, construct_filter_string,
            telemetry_init,
        },
    };
    use tracing::Level;
    ...
    let consumer = Arc::new(CaptureConsumer(Mutex::new(vec![])));
    let instance = telemetry_init(
        TelemetryOptions::builder()
            .logging(Logger::Push {
                // The routine duration line is emitted at TRACE (only the >5s line is WARN).
                filter: construct_filter_string(Level::TRACE, Level::WARN),
                consumer: consumer.clone(),
            })
            .build(),
    )
    .unwrap();
    let sub = instance.trace_subscriber().expect("subscriber present");

    let wfid = "fake_wf_id";
    // A workflow that completes in a single WFT, so that lone WFT is the reported one that
    // reaches `mark_wft_complete` and carries the metrics we attach below.
    let mut t = TestHistoryBuilder::default();
    t.add_by_type(EventType::WorkflowExecutionStarted);
    t.add_workflow_task_scheduled_and_started();
    let mock = mock_worker_client();
    let mut mock = single_hist_mock_sg(wfid, t, [1], mock, true);
    mock.set_trace_subscriber(sub);
    let core = mock_worker(mock);

    // Attach download/upload metrics to the completion, as a lang SDK would.
    let activation = core.poll_workflow_activation().await.unwrap();
    let mut completion = WorkflowActivationCompletion::from_cmds(
        activation.run_id,
        vec![CompleteWorkflowExecution { result: None }.into()],
    );
    completion.payload_download_metrics = Some(ExternalStorageMetrics {
        payload_count: 2,
    ...
    completion.payload_upload_metrics = Some(ExternalStorageMetrics {
        payload_count: 3,
    ...
    // The lone WFT's duration log must carry the completion's metrics, keeping download and
    // upload distinct (a swap or dropped field would fail here).
    let logs = consumer.0.lock().unwrap();
    let ev = logs
        .iter()
        .find(|l| l.message == "Workflow task duration information.")
        .expect("workflow task duration log");
    let u64_field = |name: &str| ev.fields.get(name).and_then(|v| v.as_u64());
    ...
    assert_eq!(u64_field("payload_download_count"), Some(2));
    assert_eq!(u64_field("payload_download_size"), Some(1024));
    assert_eq!(u64_field("payload_upload_count"), Some(3));
    assert_eq!(u64_field("payload_upload_size"), Some(2048));
    assert!(str_field("payload_download_drivers").contains("s3"));
    assert!(str_field("payload_upload_drivers").contains("gcs"));
}
