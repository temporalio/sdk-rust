...
//   heartbeats must NOT prevent start_to_close from firing locally. With the
//   buggy implementation, only the smaller of the two timeouts (heartbeat)
//   has a local timer and heartbeats keep resetting it — start_to_close is
//   never enforced locally and the activity runs indefinitely on the worker
//   even after the server has timed it out.
//
// This is the SDK-level complement to
// `worker::activities::tests::start_to_close_fires_when_heartbeat_timeout_shorter`
// (a Core unit test). This one exercises the bug end-to-end through a real
// workflow/activity against the server, observing the cancel signal at the
// activity-function level — matching the Python repro on the ticket.
//
// Sensitivity note: there's a *second* path that can cancel a "stuck"
// heartbeating activity — when the server has already timed it out, the
// next heartbeat RPC returns NotFound, and the heartbeat manager (see
// activity_heartbeat_manager.rs) turns that into a cancel. That path is
// orthogonal to the SDK-5113 fix and would mask it if heartbeats are
// frequent. The heartbeat manager throttles real RPCs to one per
// `heartbeat_timeout * 0.8` regardless of how often the activity calls
// `record_heartbeat`. So we pick `heartbeat_timeout` just under
// `start_to_close` to push the post-timeout heartbeat RPC well after
// `start_to_close`:
//   heartbeat_timeout = 4.5s  → throttle = 3.6s
//   start_to_close     = 5.0s → server times out the activity here
//   next heartbeat RPC firing after t=5s: at t=7.2s (NotFound, only after fix)
// With the fix, the local start_to_close timer fires at t≈5s.
// Without the fix, the cancel doesn't arrive until t≈7.2s (NotFound path).
// The 6.5s assertion threshold cleanly distinguishes them.
#[tokio::test]
async fn start_to_close_local_timeout_fires_with_heartbeats() {
    let wf_name = "start_to_close_local_timeout_fires_with_heartbeats";
    let mut starter = CoreWfStarter::new(wf_name);
    starter
        .set_core_cfg_mutator(|m| m.local_timeout_buffer_for_activities = Duration::from_secs(0));

    // 0 means "the activity never observed a cancel". Otherwise, milliseconds
    // from activity start to the moment cancel was observed.
    let cancel_elapsed_ms: Arc<AtomicU64> = Arc::new(AtomicU64::new(0));

    struct HeartbeatingStuckActivities {
        cancel_elapsed_ms: Arc<AtomicU64>,
    }
    #[activities]
    impl HeartbeatingStuckActivities {
        #[activity]
        async fn stuck_heartbeating_activity(
            self: Arc<Self>,
            ctx: ActivityContext,
        ) -> Result<String, ActivityError> {
            ...
            loop {
                ...
                // Wait cancellation-aware so we record the elapsed time at
                // the exact moment cancel is observed (not after the next
                // sleep wakeup).
                tokio::select! {
                    _ = ctx.cancelled() => {
                        let elapsed_ms = started.elapsed().as_millis() as u64;
                        // Floor at 1 so a (rare) sub-millisecond observation
                        // isn't mistaken for "never observed".
                        self.cancel_elapsed_ms
    ...
    starter
        .sdk_config
        .register_activities(HeartbeatingStuckActivities {
            cancel_elapsed_ms: cancel_elapsed_ms.clone(),
        });
    let mut worker = starter.worker().await;

    const START_TO_CLOSE: Duration = Duration::from_secs(5);
    const HEARTBEAT_TIMEOUT: Duration = Duration::from_millis(4500);

    #[workflow]
    #[derive(Default)]
    struct StartToCloseWithHeartbeatWorkflow;

    #[workflow_methods]
    impl StartToCloseWithHeartbeatWorkflow {
        #[run]
        async fn run(ctx: &mut WorkflowContext<Self>) -> WorkflowResult<()> {
            let res = ctx
                .start_activity(
                    HeartbeatingStuckActivities::stuck_heartbeating_activity,
                    (),
            ...
            assert_eq!(timeout.timeout_type(), TimeoutType::StartToClose);
            // Hold the worker alive past the next throttled heartbeat RPC
            // (~t=7.2s, which carries NotFound and would cancel the still-
            // running activity along that alternate path). Without this
            // trailing timer, run_until_done would shut the worker down at
            // server-side timeout (~t=5s) and the shutdown cancel would mask
            // the bug.
    ...
    worker
        .submit_workflow(
            StartToCloseWithHeartbeatWorkflow::run,
            (),
            WorkflowStartOptions::new(task_queue.clone(), task_queue).build(),
        )
        .await
    ...
    let elapsed_ms = cancel_elapsed_ms.load(Ordering::Relaxed);
    assert_ne!(elapsed_ms, 0, "activity never observed a cancel signal");
    let elapsed = Duration::from_millis(elapsed_ms);
    let threshold = START_TO_CLOSE + Duration::from_millis(1500);
    assert!(
        elapsed < threshold,
        "activity ran for {elapsed:?} before observing cancel (threshold {threshold:?}) — \
         start_to_close was not enforced locally; continuous heartbeats kept resetting the \
         only timer, leaving the bug visible only via the much-later NotFound \
         heartbeat-response path (SDK-5113)"
    );
}
