...
use temporalio_common::{
    ...
    data_converters::{
        DataConverter, DefaultFailureConverter, DefaultPayloadCodec, FailureConverter, MultiArgs1,
        MultiArgs2, PayloadCodec, PayloadConversionError, PayloadConverter, SerializationContext,
...
#[workflow]
#[derive(Default)]
struct MultiArgEndpointWorkflow {
    value: String,
    finished: bool,
}

#[workflow_methods]
impl MultiArgEndpointWorkflow {
    #[init]
    fn new(_ctx: &WorkflowContextView, input: MultiArgs2<String, ()>) -> Self {
        Self {
            value: input.0,
            finished: false,
        }
    ...
    #[run(name = "multi_arg_endpoint_workflow")]
    async fn run(ctx: &mut WorkflowContext<Self>) -> WorkflowResult<String> {
        ctx.wait_condition(|state| state.finished).await?;
        Ok(ctx.state(|state| state.value.clone()))
    }

    #[signal(name = "foreign_signal")]
    fn foreign_signal(
        &mut self,
        _ctx: &mut SyncWorkflowContext<Self>,
        input: MultiArgs2<String, ()>,
    ) {
        self.value = input.0;
    }

    #[signal(name = "foreign_unit_signal")]
    fn foreign_unit_signal(
        &mut self,
        _ctx: &mut SyncWorkflowContext<Self>,
        _input: MultiArgs1<()>,
    ) {
        self.value = "unit".to_owned();
    }

    #[signal(name = "foreign_finish")]
    fn foreign_finish(&mut self, _ctx: &mut SyncWorkflowContext<Self>) {
        self.finished = true;
    }

    #[query(name = "foreign_query")]
    fn foreign_query(&self, _ctx: &WorkflowContextView, input: MultiArgs2<String, ()>) -> String {
        format!("{}:{}", self.value, input.0)
    }

    #[update(name = "foreign_update")]
    fn foreign_update(
        &mut self,
        _ctx: &mut SyncWorkflowContext<Self>,
        input: MultiArgs2<String, ()>,
...
impl WorkflowDefinition for ForeignWorkflow {
    type Input = MultiArgs2<String, ()>;
    type Output = String;

    fn name(&self) -> &str {
        "multi_arg_endpoint_workflow"
    }
}

impl HasWorkflowDefinition for ForeignWorkflow {
    type Run = Self;
}

struct ForeignSignal;

impl SignalDefinition for ForeignSignal {
    type Workflow = ForeignWorkflow;
    type Input = MultiArgs2<String, ()>;
...
struct ForeignFinish;

impl SignalDefinition for ForeignFinish {
    type Workflow = ForeignWorkflow;
    type Input = ();
...
struct ForeignUnitSignal;

impl SignalDefinition for ForeignUnitSignal {
    type Workflow = ForeignWorkflow;
    type Input = MultiArgs1<()>;
...
impl QueryDefinition for ForeignQuery {
    type Workflow = ForeignWorkflow;
    type Input = MultiArgs2<String, ()>;
    type Output = String;
...
struct ForeignUpdate;

impl UpdateDefinition for ForeignUpdate {
    type Workflow = ForeignWorkflow;
    type Input = MultiArgs2<String, ()>;
    type Output = String;
...
#[tokio::test]
async fn multi_arg_endpoint_inputs_decode_cross_language_payloads() {
    let workflow_name = "multi_arg_endpoint_workflow";
    let mut starter = CoreWfStarter::new(workflow_name);
    starter
        .sdk_config
        .register_workflow::<MultiArgEndpointWorkflow>()
        .unwrap();
    let mut worker = starter.worker().await;
    let task_queue = starter.get_task_queue().to_owned();
    let handle = worker
        .submit_workflow(
            ForeignWorkflow,
            MultiArgs2("started".to_owned(), ()),
            WorkflowStartOptions::new(task_queue, workflow_name).build(),
        )
        .await
        .unwrap();
    ...
    let interactions = async {
        ...
        let queried = handle
            .query(
                ForeignQuery,
                MultiArgs2("queried".to_owned(), ()),
                WorkflowQueryOptions::default(),
            )
            .await
            .unwrap();
        assert_eq!(queried, "unit:queried");
        handle
            .signal(
                ForeignSignal,
                MultiArgs2("signaled".to_owned(), ()),
                WorkflowSignalOptions::default(),
            )
            .await
            .unwrap();
        let queried = handle
            ...
            .await
            .unwrap();
        assert_eq!(queried, "signaled:queried");
        let updated = handle
            .execute_update(
                ForeignUpdate,
                MultiArgs2("updated".to_owned(), ()),
                WorkflowExecuteUpdateOptions::default(),
            )
            .await
            .unwrap();
        ...
        handle
            .signal(ForeignFinish, (), WorkflowSignalOptions::default())
            .await
            .unwrap();
    };

    let (_, worker_result) = tokio::join!(interactions, worker.run_until_done());
    ...
    let input = events
        ...
        .unwrap();
    assert_eq!(input.payloads.len(), 2);
    assert_eq!(
        input.payloads[1].metadata.get("encoding").unwrap(),
        b"binary/null"
    );
    let unit_signal_input = events
        .iter()
        .find_map(|event| match event.attributes.as_ref() {
            Some(Attributes::WorkflowExecutionSignaledEventAttributes(attributes))
                if attributes.signal_name == "foreign_unit_signal" =>
            {
                attributes.input.as_ref()
            }
            _ => None,
        ...
        .unwrap();
    assert_eq!(unit_signal_input.payloads.len(), 1);
    assert_eq!(
        unit_signal_input.payloads[0]
            .metadata
            .get("encoding")
            .unwrap(),
        b"binary/null"
